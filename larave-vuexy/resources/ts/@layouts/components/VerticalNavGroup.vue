  ...
  <li v-if="canViewNavMenuGroup(item)" class="nav-group" :class="[
    ...
    <TransitionExpand>
      <ul v-show="isGroupOpen" class="nav-group-children">
        <!-- <Component :is="'children' in child && returnName(item.children) ? 'VerticalNavGroup' : VerticalNavLink"
          v-for="child in item.children" :key="child.title" :item="child">

        </Component> -->
        <div v-for="child in item.children " :key="child.title">
          <Component :is="'children' in child ? 'VerticalNavGroup' : VerticalNavLink" :item="child"
            v-if="useSectionStores.validateAccessSection(returnName(child))" />
        </div>
      </ul>
    </TransitionExpand>
  </li>
